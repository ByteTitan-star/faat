...
\section{FAAT training procedure}
\label{sec:alg-supp}
Algorithm~\ref{alg:faat} gives the full self-contained training procedure of
Sec.~3 of the main paper: the global direction $\dglobal$ is optimized first
and frozen, the poison set is chosen by the res$_x^2$ criterion, and the victim
and adaptive generator $g_\phi$ are then trained jointly. It requires only the
dataset and a target label.
...
\FOR{each epoch}
  \STATE Sample batch; form $\tilde{x}_i = \mathrm{clip}(x_i + \dglobal + g_\phi(x_i))$ for $i\!\in\!\mathcal{P}$, projecting $g_\phi(x_i)$ onto the $\ell_2$/DCT budget (Eq.~(2) of the main paper).
...
RNP~\cite{li2023rnp}           & 24.8 & 44.1 & 12.6 & 51.3 \\
I-BAU~\cite{zeng2022ibau}       & 23.1 & 87.9 & 21.4 & 88.6 \\
\bottomrule
\end{tabular}
...
with the lower-$\varepsilon$ GTSRB rows showing the stealth-but-no-ASR regime.
...
to $0.8\%$ (Fig.~9 of the main paper). On GTSRB KST stays stealthy (SSIM $0.94$
...
\input{floats/figures_tex/f9_trigger_viz}

\section{Additional visualizations}
\input{floats/figures_tex/f10_tsne}
\input{floats/figures_tex/f11_gradcam}
\input{floats/figures_tex/f13_confidence_axis}

\section{Physical robustness}
\label{sec:phys}
Figure~\ref{fig:phys} evaluates the deployed trigger under test-time input
transformations. The backdoor tracks benign accuracy under degradation: it is
not disproportionately fragile, and severe corruptions that break the attack
(e.g.\ JPEG quality $\le\!30$) also destroy benign utility.
\input{floats/figures_tex/f14_physical_robustness}
