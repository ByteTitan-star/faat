...
### 检测解耦（RQ④，量化；⚠️ 09-17 四防御补全后表述升级）
- 原 AC 反相关**保持成立**：同一 AC 检测器对**已获取**（ASR≥20）后门：GTSRB 0.77±0.10 → CIFAR-10 0.25±0.05 → CIFAR-100 **0.02±0.01（反相关）**。
- **新（`docs/pilot_defenses_ext.csv` + `docs/RQ4-DEFENSES-ANALYSIS.md`）：AC 与 STRIP 方向相反**——STRIP **正相关**（越成功越可检）：C100 a/b/cur 0.90-0.99/TPR5 0.64-0.98 全抓、GTSRB cur 0.88-0.89/TPR5 0.77 抓到；中段（CIFAR-10 全体、GTSRB a/b）0.39-0.73 看不见。**c 臂（OOD 校准）在 C100 逃逸 STRIP**（0.337/0.05，仍获取 80.5）——同数据集内唯一 <0.5 的获取臂，OOD 校准一侧杀可学性（GTSRB）、一侧赋 STRIP 规避（C100）。
- **NC/FP 结构性失效**：NC 18/18 anomaly <1（无 patch 全局触发器无异常反演）；FP90 全部 ASR 不降或 BA 同崩（C100 BA 崩 50 点后 ASR 仍 99；c 臂 FP90 7.2 系模型损毁非清后门）。
- 定稿口径：**"learnability–detectability decoupling is defense-specific"**——聚类检测反相关、熵检测正相关、OOD 校准在单数据集内翻转 STRIP 判决；反向解耦（GTSRB 非获取 c 臂 AC 0.59）保持。
